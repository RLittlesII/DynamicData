...
namespace DynamicData.Cache.Internal
{
    ...
    internal sealed class IndexCalculator<TObject, TKey>
    {
        ...
        /// <returns></returns>
        public ChangeSet<TObject, TKey> Load(ChangeAwareCache<TObject, TKey> cache)
        {
            ...
            var list = new List<KeyValuePair<TKey, TObject>>(cache.Count);
            list.AddRange(cache.KeyValues.OrderBy(kv => kv, _comparer));
            _list = list;

            var changes = new ChangeSet<TObject, TKey>(_list.Count);
            changes.AddRange(_list.Select((t, index) => new Change<TObject, TKey>(ChangeReason.Add, t.Key, t.Value, index)));
            return changes;
        }
        ...
        {
            var list = new List<KeyValuePair<TKey, TObject>>(_list.Count);
            list.AddRange(cache.KeyValues.OrderBy(kv => kv, _comparer));
            _list = list;
        }
        ...
        public ChangeSet<TObject, TKey> Reorder()
        {
            ...
            {
                int index = -1;
                foreach (var item in _list.OrderBy(t => t, _comparer))
                {
        ...
        /// </summary>
        /// <returns></returns>
        public ChangeSet<TObject, TKey> Calculate(IChangeSet<TObject, TKey> changes)
        {
            var result = new ChangeSet<TObject, TKey>(changes.Count);
            var refreshes = new ChangeSet<TObject, TKey>(changes.Refreshes);
                    ...
                    case ChangeReason.Add:
                        {
                            ...
                            result.Add(new Change<TObject, TKey>(ChangeReason.Add, change.Key, change.Current, position));
                        }
                        break;
                    ...
                    case ChangeReason.Update:
                        {
                            ...
                            result.Add(new Change<TObject, TKey>(ChangeReason.Update,
                                                                 change.Key,
                                                                 change.Current, change.Previous, newposition, old));
                        }
                        break;
                    ...
                    case ChangeReason.Remove:
                        {
                            ...
                            result.Add(new Change<TObject, TKey>(ChangeReason.Remove, change.Key, change.Current, position));
                        }
                        break;
        ...
        public List<KeyValuePair<TKey, TObject>> List => new List<KeyValuePair<TKey, TObject>>(_list);
